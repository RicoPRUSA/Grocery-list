...
const { Plus, Trash2, Check, X, ShoppingCart, Edit2 } = lucide;
...
function GroceryListApp() {
  ...
  const saveEdit = () => {
    if (editText.trim()) {
      setItems(items.map(item =>
        item.id === editingId
          ? { ...item, name: editText.trim(), quantity: editQuantity }
          : item
      ));
    }
    setEditingId(null);
          ...
          <div className="mb-6">
            <div className="flex gap-2">
              ...
              <input
                ...
                onKeyPress={handleKeyPress}
                placeholder="Qty"
                className="w-20 px-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
              />
              <button
                onClick={addItem}
...
ReactDOM.render(<GroceryListApp />, document.getElementById('root'));
